 ...
 * on a page. To help save memory it caches both computed {@link Style}s (essentially Java2D {@link
 * GeneralPath paths} for each rendered character) used to calculate character bounding boxes.
 */
public class CharacterExtractor {
   ...
   *
   * @param page the page
   * @param textRenderingMatrix The current text rendering matrix.
   * @param textMatrix The current text matrix (without font transform etc)
   * @param font The current font.
   * @param code The code point in the font.
   * @param unicode The unicode representation of the glyph in the font.
   * @param displacement The current PDF displacement.
   * @param graphicsState The current graphics state.
   * @param startText whether this is the first character in a start/end text stream
   ...
   */
  public Text calculateStyleAndPosition(
      PDPage page,
   ...
   *
   * @param at the {@link AffineTransform} that will be used to position glyphs on the page.
   ...
   *
   * @param at the transform used to position glyphs on the page.
   * @param font the {@link PDFont} to obtain glyphs from.
   * @param code the codepoint of the glyph to process
   * @param displacement the current PDF displacement {@link Vector}.
   * @return the bounding box of the character on the page.
   */
  private Rectangle2D calculateBoundingBox(
      AffineTransform at, PDFont font, int code, Vector displacement) {
    Shape path = null;
    try {
   ...
   *
   * <p>Note: if the font is not embedded and the width of the character does match the expected
   * displacement then the glyph is stretched accordingly.
   *
   * @param at the transform used to position glyphs on the page.
   * @param font the {@link PDFont} to obtain glyphs from.
   * @param displacement the current PDF displacement {@link Vector}.
   * @param path the glyph path.
   * @return the translated and displaced bounding box for the path
   */
  private static Rectangle2D createTranslatedBoundsForPath(
      AffineTransform at, PDFont font, Vector displacement, Shape path) {
   ...
   *
   * <p>From PDF 32000-1:2008 "Document management — Portable document format — Part 1: PDF 1.7"
   *
   * <blockquote>
   *
   * <p><b>9.6.4 Font Subsets</b>
   *
   * <p>For a font subset, the PostScript name of the font—the value of the font’s BaseFont entry
   ...
   * </blockquote>
   *
   * <p>We therefore strip the prefix as it is of little practical use in this application.
   *
   * @param font the font.
   ...
   * Calculate font size.
   *
   * @param textMatrix the text matrix
  ...
  private static boolean isBold(PDFont font, PDGraphicsState graphicsState) {
    ...
    boolean boldName =
        lowerCaseName.contains("black")
            || lowerCaseName.contains("bold")
