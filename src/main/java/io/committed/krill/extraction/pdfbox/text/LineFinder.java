 ...
 *
 * <p>Each {@link Line} should be semantically complete and not contain text from adjacent columns.
 */
@FunctionalInterface
public interface LineFinder {

  /**
   * Find lines in the given {@link List} of {@link PositionedContainer} of {@link Text}. The {@link
   * Text}s are to passed in the order that they were extracted from the PDF. These will be then
   * passed to a {@link WordSegmenter} to create semantic {@link Line}s of {@link Word}s.
   *
   * @param sequences the sequences of {@link Text} to process
