 ...
 *
 * <p>{@link #addPage(int, Collection, Rectangle2D)} is called to add pages to the classifier
 * (ideally all pages, as implementations may wish to compare the locations of blocks between
 * pages). Once this has been completed, {@link #label()} must be called to process the pages. Then
 * the other methods can be called to retrieve data as needed.
 */
public interface BlockTypeClassifier {
   ...
   * @param blocks the blocks for the page.
   * @param cropBox the "crop box" for the page.
   */
   ...
   *
   * @return the y coordinate of the bottom of the header region, or (note negative) -{@link
   *     Double#MAX_VALUE}, if there is no header region.
   */
