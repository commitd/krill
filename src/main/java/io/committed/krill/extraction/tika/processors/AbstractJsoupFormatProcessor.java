...
public abstract class AbstractJsoupFormatProcessor implements FormatProcessor {
   ...
   * @param document the document (which may be manipulated and returned)
   ...
   *
   * <p>Useful for adding main class="Document" around all children to signify the type.
   *
   * @param document the document
   * @param html the HTML to wrap (just like Jsoup's Element.wrap() function)
   */
   ...
   *
   * <p>Jsoup's wrap wraps each element in another tag. But often we want to wrap a group of
   ...
   *
   * @param document the document
   * @param childTag the child tag we are look for runs of
   * @param parentAlreadyValid a test to determine if a child is already correct wrapped by a parent
   * @param wrappingParentTag the parent tag to wrap with (if a valid parent is not already there)
   */
  protected void wrapRunsOfChildTag(
      final Document document,
      final String childTag,
      final Predicate<Element> parentAlreadyValid,
      final String wrappingParentTag) {
        ...
        .forEach(
            (parent, children) -> {
              ...
              for (final Element c : children) {
                if (ul == null
                    || (ul.nextElementSibling() != null && !ul.nextElementSibling().equals(c))) {
